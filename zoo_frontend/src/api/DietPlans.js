...
class DietPlan {
  ...
  async updateDietPlans(id, updates) {
    ...
    if (Object.keys(updates) === undefined || Object.keys(updates).length < 1) {
      return Promise.reject(new Error('must have object with some keys that will be updated. If meant to delete use deleteDietPlans()'));
    }
   ...
   */
  async deleteDietPlanViaFilter(filter) {
    if (!filter) {
      return Promise.reject(new Error('must have filter to be able to delete'));
    }
    const uri = `${API_BASE_URL}/api/DietPlans/deleteAllViaFilter?access_token=${this.token}`;
