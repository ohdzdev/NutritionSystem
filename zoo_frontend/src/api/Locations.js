...
class Locations {
  ...
  async updateLocations(id, updates) {
    ...
    if (Object.keys(updates) === undefined || Object.keys(updates).length < 1) {
      return Promise.reject(
        new Error(
          'must have object with some keys that will be updated. If meant to delete use deleteLocations()',
        ),
      );
    }

    const uri = `${API_BASE_URL}/Locations/${id}?access_token=${this.token}`;
  ...
  async createLocations(params, createBlank) {
    ...
    const uri = `${API_BASE_URL}/Locations/?access_token=${this.token}`;
