CREATE DEFINER=`zoo`@`%` PROCEDURE `zoo`.`GetDayDiet`(IN dayWanted VARCHAR(255))
BEGIN
	DROP TEMPORARY TABLE IF EXISTS tTodayDiet;
	DROP TEMPORARY TABLE IF EXISTS tTodayDietSub;
	DROP TEMPORARY TABLE IF EXISTS tTodayDietSubCopy;
	...
	CREATE TEMPORARY TABLE tTodayDietSubCopy
	AS
	SELECT * FROM tTodayDietSub;
		
	CREATE TEMPORARY TABLE tTodayDiet
	AS
	SELECT dayWanted AS data_date,
			tTodayDietSub.diet_id as diet_id,
			SPECIES.species as species,
			DIETS.dc_id as dc_id,
			tTodayDietSub.target as target,
			DIETS.note_id as note_id,
			DIETS.label as label,
			DELIVERY_CONTAINERS.dc as dc,
			LOCATIONS.location as location,
			tTodayDietSub.sort_order as sort_order,
			DIETS.group_id as group_id
	FROM (
			(
				(
					tTodayDietSub INNER JOIN DIETS
					ON tTodayDietSub.diet_id = DIETS.diet_id
				)
				INNER JOIN SPECIES
				ON DIETS.species_id = SPECIES.species_id
			)
	...
	GROUP BY tTodayDietSub.diet_id,
			SPECIES.species,
			DIETS.dc_id,
			tTodayDietSub.target,
			DIETS.note_id,
			DIETS.label,
			DELIVERY_CONTAINERS.dc,
			LOCATIONS.location,
			tTodayDietSub.sort_order,
			DIETS.group_id
	UNION
	SELECT dayWanted as data_date,
			tTodayDietSubCopy.diet_id as diet_id,
			"GROUP DIET" AS species,
			NULL AS dc_id,
			tTodayDietSubCopy.target as target,
